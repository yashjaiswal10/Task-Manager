...
const UIController = (function() {
    ...
    const taskOutputTemplate = ({ id, title, endAt, priority, status, completedPercentage }) => {
        return `
            <tr>
                <th scope="row">${id}</th>
                <td>${title}</td>
    ...
    return {
        ...
        populateForm({ title, subTitle, startAt, endAt, priority, status, completedPercentage }) {
        ...
        populateTask({ id, title, endAt, priority, status, completedPercentage }) {
            // Display task area when new task is added
            displayTaskArea();
            let taskResult = '';
            // Generate HTML tamplate
            taskResult += taskOutputTemplate({ id, title, endAt, priority, status, completedPercentage });
        ...
        populateAllTask(tasks) {
            ...
            tasks.forEach(task => {
                ...
                const { id, title, priority, status, endAt, completedPercentage } = task;
                // Generate HTML tamplate
                tasksResult += taskOutputTemplate({ id, title, endAt, priority, status, completedPercentage });
            });
